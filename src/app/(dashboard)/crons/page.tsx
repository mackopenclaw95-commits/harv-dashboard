...
export default function AutomationsPage() {
  ...
  useEffect(() => {
    ...
    const refreshCustom = () => setCustomAutomations(getCustomAutomations());
    window.addEventListener("tour-open-templates", openTemplates);
    window.addEventListener("tour-close-templates", closeTemplates);
    window.addEventListener("tour-refresh-automations", refreshCustom);
    return () => {
      window.removeEventListener("tour-open-templates", openTemplates);
      window.removeEventListener("tour-close-templates", closeTemplates);
      window.removeEventListener("tour-refresh-automations", refreshCustom);
    };
  }, []);
    ...
    <div className="p-6 space-y-8 max-w-5xl mx-auto">
      ...
      <div>
        ...
        {loading ? (
          ...
          <div data-tour="crons-grid" className="grid gap-4 sm:grid-cols-2 items-start">
            {activeJobs.map((job) => {
            ...
            {/* Custom automations */}
            {customAutomations.map((auto, idx) => (
              <Card key={auto.id} className="transition-all duration-300 hover:ring-primary/15" {...(auto.id === "tour-demo" ? { "data-tour": "crons-demo-card" } : {})}>
                <CardContent className="pt-5 pb-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
      ...
      {showTemplates && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowTemplates(false)}>
          ...
          >
            ...
            {/* Template grid */}
            <div data-tour="crons-templates" className="p-6 grid gap-3 sm:grid-cols-2">
              {TEMPLATES.map((t) => (
                <button
                  key={t.id}
